...
pub fn query_swap_status(deps: Deps, swap_id: String) -> StdResult<SwapStatusResponse> {
    ...
    Ok(SwapStatusResponse { swap_id, status })
}
...
pub fn query_swap(deps: Deps, swap_id: String) -> StdResult<SwapResponse> {
    ...
    Ok(SwapResponse { swap_id, swap_data })
}
...
pub fn query_all_swaps(
    ...
    let start = start_after.as_deref().map(|s| Bound::exclusive(s));
    ...
    Ok(SwapListResponse { swaps: swaps? })
}
...
pub fn query_all_src_escrows(
    ...
    Ok(SrcEscrowListResponse { escrows: escrows? })
}
...
pub fn query_validate_escrow(
    ...
    let is_valid = SRC_ESCROWS.has(deps.storage, escrow_address.clone())
        || DST_ESCROWS.has(deps.storage, escrow_address);
...
pub fn query_all_dst_escrows(
    ...
    Ok(DstEscrowListResponse { escrows: escrows? })
}
